...
int background_functions(
  ...
  /* change in rho_tot */  /* Poudou */

  /*Gaussian function for gaussian changes*//*Poudou*/
  double gaussian_change (double amp, double m, double s){
    double z=1/a-1;
    double gauss=amp*exp(-pow(z-m,2)/(2*pow(s,2)))/sqrt(2*M_PI*pow(s,2));
    return gauss;
  }

  if (pba->has_gauss_change == _TRUE_) {
    rho_tot = rho_tot*(1+gaussian_change(pba->gauss_amplitude,pba->z_mean,pba->sigma_change));
    //printf("valeur de la gausienne %f\n", gaussian_change(pba->gauss_amplitude,pba->z_mean,pba->sigma_change));
  }

  //printf("valeur de z_mean %f,", pba->z_mean);
  //printf("valeur de sigma_change %f,", pba->sigma_change);
  //printf("y a t-il gauss_change %d,", pba->has_gauss_change);
  //printf("valeur de la gausienne %f,%f,%f\n", gaussian_change(pba->gauss_amplitude,pba->z_mean,pba->sigma_change),a,pba->z_mean);
  //printf("valeur de 1/z_mean %f\n", 1/(pba->a_mean));
  ...
  //if (pba->has_gauss_change == _TRUE_) {
    //pvecback[pba->index_bg_H] = pvecback[pba->index_bg_H]*(1+gaussian_change(pba->gauss_amplitude,pba->z_mean,pba->sigma_change));
  //}

  //printf("%f,",pvecback[pba->index_bg_H]);
